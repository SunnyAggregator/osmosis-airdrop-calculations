...
import { osmosisAddressToBuffer } from "./osmosisAddress";
...
export function parseBalanceMap(balances: NewFormat[]): MerkleDistributorInfo {
  ...
  const tree = new BalanceTree(
    sortedAddresses.map((address) => {
      ...
      return {
        account: osmosisAddressToBuffer(address),
  ...
  const claims = sortedAddresses.reduce<{
    ...
    memo[address] = {
      index,
      amount: amount,
      proof: tree.getProof(index, osmosisAddressToBuffer(address), amount),
      ...(flags ? { flags } : {}),
    };
    return memo;
  }, {});
